...
new Handle:g_h_match_can_clinch = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_h_veto = CreateConVar("wm_veto", "0", "Veto Style: 0 = Bo1, 1 = Bo3", FCVAR_NOTIFY);
	...
	g_h_match_can_clinch = FindConVar("mp_match_can_clinch");
...
public Action:ConsoleScore(client, args)
{
	...
	if (g_match)
	{
		...
		PrintToConsole(client, "\x01 \x09[\x04%s\x09]\x01  %s: [%d] %s: [%d] MR%d", CHAT_PREFIX, g_t_name, GetTScore(), g_ct_name, GetCTScore(), (GetConVarInt(g_h_max_rounds)/2));
		if (g_overtime)
		{
			PrintToConsole(client, "\x01 \x09[\x04%s\x09]\x01 %t (%d): %s: [%d], %s: [%d] MR%d", CHAT_PREFIX, "Score Overtime", g_overtime_count + 1, g_t_name, GetTOTScore(), g_ct_name, GetCTOTScore(), (GetConVarInt(g_h_overtime_mr)/2));
		}
	}
	else
	{
...
//			ServerCommand("exec %s", half_time_config);
		...
		else if (GetTScore() == (GetConVarInt(g_h_max_rounds)/2) && GetCTScore() == (GetConVarInt(g_h_max_rounds)/2)) // complete draw
		{
			...
			{ // max rounds overtime
				...
				DisplayScore(0, 0, false);
				PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 %t", CHAT_PREFIX, "Over Time", (GetConVarInt(g_h_overtime_mr)/2));
//					g_live = false;
				g_t_money = false;
				g_overtime = true;
				g_overtime_mode = 1;
		...
		else if (GetScore() == GetConVarInt(g_h_max_rounds)) // full time (all rounds have been played out)
		{
			Call_StartForward(g_f_on_end_match);
			Call_Finish();
		...
		else if (g_h_match_can_clinch && GetTScore() == (GetConVarInt(g_h_max_rounds)/2) + 1 || GetCTScore() == (GetConVarInt(g_h_max_rounds)/2) + 1) // full time
		{
			DisplayScore(0, 0, false);
			PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 %t", CHAT_PREFIX, "Full Time");
			
			if (g_h_match_can_clinch)
			{
				Call_StartForward(g_f_on_end_match);
				Call_Finish();
	...
	{
		if (GetOTScore() == (GetConVarInt(g_h_overtime_mr)/2)) // half time
		{
			if (!g_first_half)
			{
				return;
		...
		else if (GetTOTScore() == (GetConVarInt(g_h_overtime_mr)/2) && GetCTOTScore() == (GetConVarInt(g_h_overtime_mr)/2)) // complete draw
		{
			...
			else if (GetTOTScore() == (GetConVarInt(g_h_overtime_mr)/2) + 1 || GetCTOTScore() == (GetConVarInt(g_h_overtime_mr)/2) + 1) // full time
			{
				Call_StartForward(g_f_on_end_match);
				Call_Finish();
...
public Action:ShowPluginInfo(Handle:timer, any:client)
{
	if (client != 0 && IsClientConnected(client) && IsClientInGame(client))
	{
		...
		PrintToConsole(client, "");
		PrintToConsole(client, "Current settings: %s: %d / %s: %d / mp_match_can_clinch: %d", max_rounds, GetConVarInt(g_h_max_rounds), min_ready, GetConVarInt(g_h_min_ready), GetConVarInt(g_h_match_can_clinch));
		PrintToConsole(client, "===============================================================================");
	}
}
...
public Action:Veto_Bo3_Maps(client, args)
{
	...
	{
		PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 No maps in bo3 list", CHAT_PREFIX);
		return;
	}
	...
	PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 Map 1 = %s, Map 2 = %s, Map 3 = %s", CHAT_PREFIX, map[0], map[1], map[2]);
}
...
public Action:Veto_Setup(client, args)
{
	...
	{
		PrintToChat(client, "\x01 \x09[\x04%s\x09]\x01 You must be on Terrorist or Counter Terrorist to use !veto", CHAT_PREFIX);
		return;
	}
	...
	{
		PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 Counter Terrorist have asked for veto", CHAT_PREFIX);
		PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 Please type !veto to start", CHAT_PREFIX);
		g_h_stored_timer_v = CreateTimer(30.0, VetoTimeout);
	}
	else if (!veto_offer_ct && veto_offer_t)
	{
		PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 Terrorist have asked for veto", CHAT_PREFIX);
		PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 Please type !veto to start", CHAT_PREFIX);
		g_h_stored_timer_v = CreateTimer(30.0, VetoTimeout);
	}
}
...
public SetCapt1(client)
{
	...
	{
		g_capt1 = client;
		PrintToChatAll("Captain for Terrorists will be %N", g_capt1);
	}
}
...
public SetCapt2(client)
{
	...
	{
		g_capt2 = client;
		PrintToChatAll("Captain for Counter-Terrorists will be %N", g_capt2);
	}
}
...
public VetoHandler(Handle:menu, MenuAction:action, param1, param2) {
	if (action == MenuAction_Select) {
		...
		if (!g_veto_s)
		{
			...
			if (GetConVarInt(g_h_veto) == 1 && GetConVarInt(g_h_veto_bo3) == 1 && g_veto_number == 2)
			{
				new other = OtherCaptain(client);
				GiveVetoMenuSelect(other);
				DisplayVeto(other);
			}
			else if (GetConVarInt(g_h_veto) == 1 && GetConVarInt(g_h_veto_bo3) == 1 && (g_MapListCount - 2))
			{
			...
			else if (GetConVarInt(g_h_veto) == 1 && g_veto_number == (g_MapListCount - 3))
			{
...
public Action:MatchRestore(client, const String:command[], args)
{
	...
	if (ot_count > 1)
	{
		...
		{
			g_scores_overtime[SCORE_T][i][SCORE_FIRST_HALF] = ((GetConVarInt(g_h_overtime_mr)/2)/2);
			g_scores_overtime[SCORE_CT][i][SCORE_FIRST_HALF] = ((GetConVarInt(g_h_overtime_mr)/2)/2+1);
			SwitchScores();
			g_scores_overtime[SCORE_T][i][SCORE_SECOND_HALF] = ((GetConVarInt(g_h_overtime_mr)/2)/2+1);
			g_scores_overtime[SCORE_CT][i][SCORE_SECOND_HALF] = ((GetConVarInt(g_h_overtime_mr)/2)/2);
			team1_ot = team1_ot - (GetConVarInt(g_h_overtime_mr)/2);
			team2_ot = team2_ot - (GetConVarInt(g_h_overtime_mr)/2);
			g_overtime_count++;
		}
	}
	...
	if (team1_ot + team2_ot >= (GetConVarInt(g_h_overtime_mr)/2))
	{
		g_first_half = false;
	}
